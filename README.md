...
[media-release下载](http://say-public.oss-cn-beijing.aliyuncs.com/cosmos/media-release.aar)

[recordsdk-release下载](http://say-public.oss-cn-beijing.aliyuncs.com/cosmos/recordsdk-release.aar)
...
SDK支持单一架构，请在app/build.gradle文件中配置如下代码。在android/defaultConfig/结构下添加
...
### 注册
    ...
    com.immomo.mediasdk.MoMediaManager#register(Application application, String appid, RegisterCallback callback);

    
### 获取录制器

    IMultiRecorder com.immomo.mediasdk.MoMediaManager#createRecorder(String token);



### 获取视频处理器

    IVideoProcessor com.immomo.mediasdk.MoMediaManager#createVideoProcessor(String token);

    

### 获取图片处理器

    ImageProcess com.immomo.mediasdk.MoMediaManager#createImageProcessor(String token);
