...
import com.immomo.videosdk.recorder.adapter.CoverListAdapter;
import com.immomo.videosdk.recorder.model.Video;
import com.immomo.videosdk.utils.SaveImageManager;
...
public class SelectMomentCoverActivity extends BaseFullScreenActivity implements View.OnClickListener, SaveImageManager.Callback {
    ...
    private void initFrameCount(long duration) {
        if (duration > 0 && duration <= MediaConstants.ONE_MINUTE) {
            frameCount = 10;
        } else if (duration > MediaConstants.ONE_MINUTE && duration <= 3 * MediaConstants.ONE_MINUTE) {
            frameCount = 20;
        } else
            frameCount = 30;
    }
