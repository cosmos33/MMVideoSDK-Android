...
import com.immomo.videosdk.recorder.sticker.StickerIDUtils;
...
public class VideoEditPresenter implements IProcessPresenter {
    ...
    public VideoEditPresenter(@NonNull IProcessPlayerView view, @NonNull Video video) {
        ...
        videoProcessor.setOutVideoInfo(size[0], size[1], fps, bitrate, isUseCQ);
    }
    ...
    public void makeVideo(boolean forceMake) {
        ...
        File cache = RecoderFileUtils.getRecoderCache();
        if (cache == null) {
            return;
        }

        final File destFile = new File(RecoderFileUtils.getRecoderCache(), System.currentTimeMillis() + ".mp4_");
        ...
        videoProcessor.setUseBgChanger(momentExtraInfo.getUseBgChanger());
        //添加文字贴纸或者水印
        BasicFilter textFilter = getTextFilter(momentExtraInfo.getBlendBitmap());

        if (textFilter != null) {
            videoProcessor.addFilters(textFilter);
        }
