...
import com.immomo.videosdk.widget.videorangebar.RangeBarListener;
import com.immomo.videosdk.widget.videorangebar.VideoRange;
import com.immomo.videosdk.widget.videorangebar.VideoRangeBar;
...
import static com.immomo.videosdk.recorder.MediaConstants.KEY_CUT_VIDEO_RESULT;
import static com.immomo.videosdk.recorder.MediaConstants.KEY_PICKER_VIDEO;
...
public class VideoCutFragment extends BaseFragment implements View.OnClickListener {
    ...
    MomoProcess process = new MomoProcess();
    ...
    private void initData(Bundle arg) {
        if (null != arg) {
            ...
            initMinDuration = arg.getInt(VideoRecordDefs.VIDEO_MIN_CUT_TIME, initMinDuration);
    ...
    private void initProcess() {
        ...
        process.setPlayingStatusListener(new MRecorderActions.OnPlayingStatusListener() {
            ...
            public void onPlayingPaused() {
                Log4Android.getInstance().i("yichao------onPlayingPaused 播放暂停");
    ...
    private void initSurfaceView() {
        ...
        effectModel.setMediaPath(currentVideo.path);
        effectModel.setVideoEffects(new VideoEffects());
        effectModel.setAudioEffects(new AudioEffects());
        String json = EffectModel.toEffectCmd(effectModel);
        Log4Android.getInstance().i(" json:" + json);
        processPrepred = process.prepare(json);
    ...
    private int getStep() {
        ...
        } else if (currentVideo.length <= 2 * MediaConstants.ONE_MINUTE) {
    ...
    private void cutError(final MomoProcess cutProcess) {
    ...
    private void startPlay(long startTime, long endTime) {
        String json = getVideoCutJson(startTime, endTime);
        videoRangeBar.scrollToTimestamp(startTime, true);
        Log4Android.getInstance().i("json:" + json);
        if (process != null) {
            process.updateEffect(json, 0,true);
        }
    }

    private String getVideoCutJson(long startTime, long endTime) {
        startPosMs = startTime;
        EffectModel effectModel = new EffectModel();
        effectModel.setMediaPath(currentVideo.path);
        ...
        effectModel.setVideoEffects(videoEffects);
        effectModel.setAudioEffects(new AudioEffects());
        return EffectModel.toEffectCmd(effectModel);
    }
    ...
    private static class UIHandler extends com.immomo.videosdk.utils.UIHandler<VideoCutFragment> {
                ...
                switch (msg.what) {
                    case ACTION_UPDATE_RANGEBAR:
                        if (fragment.videoRangeBar != null) {
                            ...
                            String time = null;
                            if (fragment.initMinDuration >= MediaConstants.ONE_MINUTE) {
                                time = fragment.initMinDuration / MediaConstants.ONE_MINUTE + "分钟";
                            } else {
