...
import com.immomo.videosdk.recorder.activity.SelectMomentCoverActivity;
import com.immomo.videosdk.recorder.activity.VideoCutActivity;
import com.immomo.videosdk.recorder.activity.VideoSpeedAdjustActivity;
import com.immomo.videosdk.recorder.adapter.DynamicStickerListAdapter;
import com.immomo.videosdk.recorder.editor.EditRecorder;
import com.immomo.videosdk.recorder.editor.player.IProcessPlayerView;
import com.immomo.videosdk.recorder.editor.player.IProcessPresenter;
import com.immomo.videosdk.recorder.editor.player.OnVolumeChangeListener;
import com.immomo.videosdk.recorder.editor.player.VideoEditPresenter;
import com.immomo.videosdk.recorder.helper.MomentVideoProcessor;
...
import com.immomo.videosdk.recorder.listener.StickerEditListener;
import com.immomo.videosdk.recorder.model.MusicContent;
import com.immomo.videosdk.recorder.model.Video;
import com.immomo.videosdk.recorder.musicpanel.edit.MusicPanelHelper;
import com.immomo.videosdk.recorder.specialfilter.ISpecialDataControl;
import com.immomo.videosdk.recorder.specialfilter.SpecialDataControl;
import com.immomo.videosdk.recorder.specialfilter.view.SpecialPanelViewHelper;
...
import com.immomo.videosdk.utils.AlbumNotifyHelper;
import com.immomo.videosdk.utils.AnimUtils;
import com.immomo.videosdk.utils.DeviceUtils;
import com.immomo.videosdk.utils.KeyBoardUtil;
import com.immomo.videosdk.utils.OnAnimationEndListener;
...
import com.immomo.videosdk.widget.DynamicStickerPanel;
import com.immomo.videosdk.widget.IndeterminateDrawable;
...
import com.immomo.videosdk.widget.progress.CircleProgressView;
...
public class VideoEditFragment extends BaseFragment
    ...
    private boolean uploadNeedWaterMask;
    private boolean globalConfigNeedWaterMask;
    private boolean isDoLastSave;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        MomoProcess.InitGloabalEglResource();
        if (transBean == null) {
    ...
    private void initWaterMaskMode() {
        //        取消灰度，所以视频上传到服务器不加水印
        uploadNeedWaterMask = false;
        if (transBean != null && transBean.needWaterMask()) {
            globalConfigNeedWaterMask = true;
        } else {
            globalConfigNeedWaterMask = false;
        }
    }

    private void getCoverByVideo() {
        MomentVideoProcessor.generateThumbnail(video.path, video.rotate, new MomentVideoProcessor.ThumbnailCallback() {
            @Override
    ...
    public void onDestroy() {
        ...
        MomoProcess.UinitGloabalEglResource();
    ...
    private void prepareEncoding(boolean isSend, boolean saveToGallery, boolean secondCompress) {
        ...
        if (stickerWidth > 0 && stickerHeight > 0) {
            ...
            if ((isSend && globalConfigNeedWaterMask && uploadNeedWaterMask) || (!isSend && globalConfigNeedWaterMask) || isDoLastSave) {

                if (blendBitmap == null)
                    blendBitmap = Bitmap.createBitmap(stickerWidth, stickerHeight, Bitmap.Config.ARGB_8888);
                //                blendBitmap = MicroVideoWaterMarkHelper.getBlendBitmapByVideoSize(blendBitmap, video.width, video.height);
                blendChanged = true;
            }
        ...
        boolean hasVideoChanged;
        boolean onlyAudioChanged = false;
        if (isVideoRed() && !secondCompress) {
            if (video.size <= 60 * 1024 * 1024 && !isUsedSpecialFilter() && !editRecorder.isOnlyChangeVideo()
                    && !blendChanged && !isAudioPitch()) {
                //红人视频上传逻辑(1、是红人 2、去发布页面 3、不是第二次压缩 4、视频没有旋转过 5、视频小于60Mb 6、没有添加变音)
            ...
            hasVideoChanged = isVideoEdited() || blendChanged || video.frameRate > MediaConstants.MAX_VIDEO_FRAME_RATE  // 是否编辑过
                    || !video.isChosenFromLocal                                                                             // 录制的视频是否要压缩
                    || ((transBean != null && transBean.needWaterMask()) && secondCompress)                                 // 是否有水印
                    || isNeedTranscode(transBean)
                    || video.playingMusic != null;
        }

        if (isVideoRed() && !secondCompress) {
            //第一次压缩时,如果是红人走一遍软编码逻辑
            setEncode();
        }
        //如果配乐不为空，且视频未经过任何其它的编辑情况下，进行如下处理
        if (hasVideoChanged == false && video.playingMusic != null) {
            hasVideoChanged = true;
    ...
    private void setEncode() {
        //服务器下发
        int result = MomoProcess.MEDIA_ENCODER_MODE_CODEC;
        processPresenter.getMomentExtraInfo().setEncodeMode(result);
    }

    private void doLastSaveIfNeed(boolean isSend) {
        MDLog.i(LogTag.PROCESSOR.PROCESS, "doLastSaveIfNeed isSend:" + isSend);
        updateWaterMarkAndTextBitmap(isSend);
        processPresenter.setProcessListener(new VideoEditPresenter.OnProcessListener() {
            @Override
            public void onProcessProgress(float progress) {
                MDLog.i(LogTag.PROCESSOR.PROCESS, "onProcessProgress");
            }

            @Override
            public void onProcessFinish(String videoPath) {
                MDLog.e(LogTag.PROCESSOR.PROCESS, "onProcessFinish" + videoPath);
            ...
            @Override
            public void onProcessStart() {
                MDLog.e(LogTag.PROCESSOR.PROCESS, "onProcessStart");
            }

            @Override
            public void onProcessFailed(String ex) {
                MDLog.e(LogTag.PROCESSOR.PROCESS, "onProcessFailed");
                processPresenter.onDestroy();
            }
        });

        if (processPresenter.getMomentExtraInfo() != null) {
            processPresenter.getMomentExtraInfo().setUseBgChanger(false);
        }
        processPresenter.makeVideo(true);
    }

    private void updateWaterMarkAndTextBitmap(final boolean isSend) {
        Bitmap blendBitmap = null;
        if (stickerWidth > 0 && stickerHeight > 0) {
            if (allStickerContainer != null && stickerContainerView != null && editRecorder.isChangeFilter()) {
                blendBitmap = ImageUtil.createBitmapByView(allStickerContainer,
    ...
    private void returnVideo(File file, boolean isSend) {
        if (getActivity() != null && !getActivity().isFinishing()) {
            if (globalConfigNeedWaterMask) {
                isDoLastSave = true;
                doLastSaveIfNeed(isSend);
            } else {
                processPresenter.onDestroy();
            }
            video.path = file.getAbsolutePath();
    ...
    private String[] getDecoratorText() {
        if (textStickerView != null && textStickerView.size() > 0) {
            final int len = textStickerView.size();
            String[] decoratorText = new String[len];
            for (int i = 0; i < len; i++) {
                String text = "";
                StickerView sv = textStickerView.get(i);
                if (sv != null) {
                    text = sv.getText();
                }
                decoratorText[i] = text;
            }
