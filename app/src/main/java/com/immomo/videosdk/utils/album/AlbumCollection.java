...
import com.immomo.videosdk.recorder.model.LatLonPhoto;
...
public class AlbumCollection implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private final VideoInfoTransBean mTransBean;
    ...
    public AlbumCollection(@NonNull VideoInfoTransBean conditions, int mediaType, @NonNull FragmentActivity context, @NonNull OnMediaListener listener) {
    ...
    private ScanResult getAlbumDirectories(Cursor data,
        ...
        //8.10.3相册引导，需要将关注列表和个人动态列表，带过来的图片，放到相册最前面展示。
        final List<LatLonPhoto> latLonPhotos = mTransBean.hasLatLonPhotos ?
                (MediaSourceHelper.sLatLonMedias == null ? null : MediaSourceHelper.sLatLonMedias.photoList) : null;
        List<Photo> newPosPhoto = null;
        long recentTime = 0;
        long lastTime = 0;
        int index = 0;
        boolean needCheck = false;
        if (latLonPhotos != null && latLonPhotos.size() > 0) {
            needCheck = true;//needCheck为true，表示需要走相册引导扫描逻辑，否则走之前的扫描逻辑。
            newPosPhoto = new ArrayList<>(latLonPhotos.size());
            recentTime = latLonPhotos.get(0).dateAdded;
            lastTime = latLonPhotos.get(latLonPhotos.size() - 1).dateAdded;
        }
        ...
        do {
            ...
            if (needCheck) {
                if (media.dateAdded > recentTime || media.dateAdded < lastTime) {
                    directoryAll.getMedias().add(media);
                } else {
                    //将新地点的图片插入到集合最前面。
                    if (TextUtils.equals(MD5Utils.getMD5(media.path), latLonPhotos.get(index).guid)) {
                        newPosPhoto.add(media);
                        if (++index >= latLonPhotos.size()) {
                            needCheck = false;
                            newPosLength = newPosPhoto.size();
                            directoryAll.getMedias().addAll(0, newPosPhoto);
                        }
                    } else {
                        directoryAll.getMedias().add(media);
                    }
    ...
    private Photo parseFromCursorFast(Cursor cursor) {
        ...
        if ((!isImage || (isGif && !mTransBean.gifEnable)) && !isVideo) {
            return null;
        }
