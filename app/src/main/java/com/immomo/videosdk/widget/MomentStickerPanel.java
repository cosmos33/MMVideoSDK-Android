...
import com.immomo.videosdk.widget.decoration.SpaceItemDecoration;
import com.immomo.videosdk.glide.ImageLoaderX;
import com.immomo.videosdk.recorder.adapter.MomentStickerListAdapter;
...
public class MomentStickerPanel extends FrameLayout {
    ...
    private ArrayList<MomentSticker> stickerList = new ArrayList<>();
    ...
    private void onLoadSuccess(ArrayList<MomentSticker> list) {
        if (errorTipView != null) {
            errorTipView.setVisibility(View.GONE);
        }
        showProgress(false);
    ...
    private class LoadStickerTask extends MomoTaskExecutor.Task<Object, Object, ArrayList<MomentSticker>> {
        @Override
        protected ArrayList<MomentSticker> executeTask(Object... params) throws Exception {
            ArrayList<MomentSticker> list = new ArrayList<>();
            //贴纸过期或者缓存无效时，需要重新拉取一次数据
            MoApi.getStaticStickerList(list);
            return list;
        }

        @Override
        protected void onTaskSuccess(ArrayList<MomentSticker> list) {
