...
import com.immomo.videosdk.widget.paint.DrawableViewConfig;
    ...
    if (insideCanvas(touchX, touchY)) {
      ...
      if (config != null) {
        currentDrawingPath.setColor(config.getStrokeColor());
          currentDrawingPath.setShader(config.getShader());
        currentDrawingPath.setWidth(config.getStrokeWidth());
        currentDrawingPath.setPenType(config.getPenType());
      }
