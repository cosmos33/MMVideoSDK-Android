...
public class MomentFace implements Cloneable, Parcelable {
    ...
    public static MomentFace fromJson(JSONObject json) {
        ...
        try {
            ...
            face.tag = json.optString("tag");
            face.tagColor = json.optString("tag_color");
            face.sound = json.optInt("sound");
            face.isFaceRig = json.optInt("is_facerig") == 1;
            face.isArkit = json.optInt("is_arkit") == 1;
            face.isOverlap = json.optInt("is_overlap") == 1;
            return face;
    ...
    public static JSONObject toJson(MomentFace face) {
        ...
        try {
            ...
            json.put("tag", face.tag);
            json.put("tag_color", face.tagColor);
            json.put("sound", face.sound);
            json.put("is_facerig", face.isFaceRig ? 1 : 0);
            json.put("is_arkit", face.isArkit ? 1 : 0);
            json.put("is_overlap", face.isOverlap ? 1 : 0);
            return json;
        } catch (JSONException e) {
    ...
    public MomentFace clone() {
        ...
        if (face == null) {
            ...
            face.tag = this.tag;
            face.tagColor = this.tagColor;
            face.sound = this.sound;
        }
        return face;
    }
