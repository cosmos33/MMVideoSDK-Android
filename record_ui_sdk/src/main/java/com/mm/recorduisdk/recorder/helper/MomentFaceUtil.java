...
import com.mm.recorduisdk.moment.MyClassDataProcessor;
...
public class MomentFaceUtil {
    ...
    public static MomentFaceDataManager createMomentFaceDataManager(@MomentPanelType int faceType) {
        MomentFaceDataManager momentFaceDataManager;
                ...
                // 设置数据加工器
                momentFaceDataManager.addDataProcessor(new MyClassDataProcessor());
     ...
     * @param faceId 变脸元素id
     */
    public static MomentFace findMomentFace(List<FaceClass> classList, String classId, String faceId) {
        if (classList == null || classList.isEmpty()) {
            return null;
        }
        for (FaceClass faceClass : classList) {
            if (faceClass.getId().equals(classId)) {
                List<MomentFace> faceList = faceClass.getFaces();
                if (faceList != null) {
                    for (MomentFace momentFace : faceList) {
                        if (momentFace.getId().equals(faceId)) {
                            return momentFace;
                        }
                    }
                }
     ...
     * @return true 正在下载
     */
    public static boolean isFaceResourceDownloading(MomentFace face) {
        return MomentFaceDownloader.getInstance().isFaceResourceDownloading(face);
    }

    /**
     * 是否正在下载队列中，可能在下载中，或者等待下载的状态，如果想知道是否在下载，{@link #isFaceResourceDownloading(MomentFace)}
     * @return true 在队列里
     */
