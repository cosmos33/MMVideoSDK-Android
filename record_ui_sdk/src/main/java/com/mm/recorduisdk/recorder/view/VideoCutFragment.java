...
import static com.mm.recorduisdk.recorder.MediaConstants.KEY_CUT_VIDEO_RESULT;
import static com.mm.recorduisdk.recorder.MediaConstants.KEY_PICKER_VIDEO;
...
//        Log4Android.getInstance().i("tang------onPlaying " + (curPos));
    ...
    public void onClick(View v) {
        ...
        } else if (videoRangeBar == v) {
            toast("缩略图生成中...");
        }else if(v==cut_btn_play){
            if (isPlaying) {
                isPlaying = false;
                isHandPause = true;
                if (process != null) {
    ...
    private void startPlay(long startTime, long endTime) {
        ...
        if (process != null) {
            process.updateEffect(videoCuts, null, 0, true);
        }
    }
    ...
    private final CutUIHandler handler = new CutUIHandler(this);

    private static class CutUIHandler extends UIHandler<VideoCutFragment> {

        public CutUIHandler(VideoCutFragment cls) {
            super(cls);
        }
